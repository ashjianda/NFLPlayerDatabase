...
<div class="bg-dark pb-2" style="background-color: #e3f2fd;">
    <a href="compare.php" class="btn btn-outline-light mt-2 ml-2">Search Another Player</a>
</div>
<?php
function stats($player){
    ...
    $stmtName->bindValue(':player_id', $player);
    $stmtName->execute();
    $rowName = $stmtName->fetch();

    $SQLQueryHistory = "SELECT team_name, MIN(season) as 'start', MAX(season) as 'end', GROUP_CONCAT(DISTINCT jersey_number) as 'jersey' FROM `plays` GROUP BY player_id, team_name HAVING player_id = :player_id ORDER BY start";
    $stmtHistory = $conn->prepare($SQLQueryHistory);
    $stmtHistory->bindValue(':player_id', $player);
    $stmtHistory->execute();
    ...
    $stmtOffense->bindValue(':player_id', $player);
    $stmtOffense->execute();

    $stmtTotOffense = $conn->prepare($SQLOffenseTotal);
    $stmtTotOffense->bindValue(':player_id', $player);
    ...
    $stmtDefense->bindValue(':player_id', $player);
    $stmtDefense->execute();
    ...
    $stmtTotDefense->bindValue(':player_id', $player);
    ...
    $stmtP->bindValue(':player_id', $player);
    $stmtPR->bindValue(':player_id', $player);
    $stmtK->bindValue(':player_id', $player);
    $stmtKR->bindValue(':player_id', $player);
    ...
    $stmtPTot->bindValue(':player_id', $player);
    $stmtPTot->execute();
    ...
    $stmtKTot->bindValue(':player_id', $player);
    $stmtKTot->execute();
    ...
    $stmtPRTot->bindValue(':player_id', $player);
    $stmtPRTot->execute();
    ...
    $stmtKRTot->bindValue(':player_id', $player);
    $stmtKRTot->execute();
    ...
    $stmt2023->bindValue(':player_id', $player);
    $stmt2023->execute();
    ...
    <div class="row g-0 d-flex align-items-end" style="background-color: gray">
        <div class="col">
            <h1 class="pl-2 display-6"><?php echo $rowName['name'];?></h1>
        </div>
    </div>
    <div class="row g-0 d-flex align-items-end" style="background-color: gray">
        <img class="img-fluid" style="height: 550px" src="<?php echo $rowName['headshot'];?>">
    </div>
    <div class="row ml-0 g-0 d-flex align-items-end" style="background-color: gray">
    ...
    if($numberRowsOffense > 0){?>
        ...
        <table class="ml-4 table table-bordered table-sm table-responsive" style="max-width: 95%">
    ...
    if($numberRowsDefense > 0){?>
        ...
        <table class="ml-4 table table-bordered table-sm table-responsive" style="max-width: 97%">
            <thead class="thead-dark">
                <tr>
                    <th>Season</th>
                    <th>Team</th>
...
<div class="row">
    <div class="col-6 pr-0">
        <?php stats($_POST['player_id']); ?>
    </div>
    <div class="col-6 pl-0">
        <?php stats($_POST['s_player_id']); ?>
    </div>
</div>